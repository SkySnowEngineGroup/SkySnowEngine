...
1.1 Improvements and New Features
    ...
    [FBXX-1641] Added sample code to give an experimental example of referencing FBX files.
                Please read the full comments in the ExternalDocsRefs sample code for
                the constraints and limitations.

1.2 Changes and Deprecated Features

    [FBXX-1664] Generate FBX Python Bindings with Python v3.10.x interpreter
    [FBXX-1663] Make sure Sip code can be compiled with VS2022
    [FBXX-1658] Updated zlib to version 1.2.13

    [FBXX-1667] Remove support for VS2015
    Visual Studio 2017, 2019 and 2022 are supported.

    [FBXX-1527] remove 32 bit build configurations
    Support for 32 bit configurations have been removed
...
2.1 Fixed Issues
    ...
    [FBXX-1661] Security improvements
    [FBXX-1655] Exporter does not complete task with pNonBlocking option
    [FBXX-1652] removed unnecessary throw call in fbxlayer.h
    [FBXX-1649] resolved detecting false positives errors while reading FbxShape normals
    [FBXX-1644] Legacy triangulate function in FbxgeometryConverter crashes when no materials are defined
...
2020.3.2
    ...
    [FBXX-1640] FBX SDK libraries compiled with VisualStudio 2019 targeting x64 architecture are now
                using the EH Continuation (EHCONT) flag. For more information please refer to Microsoft
                documentation: https://github.com/MicrosoftDocs/cpp-docs/blob/main/docs/build/reference/guard-enable-eh-continuation-metadata.md

    [FBXX-1649] Fixed issue while reading FbxShape normals
    [FBXX-1644] Legacy triangulate algorithm can cause crashes
    [FBXX-1642] Issues with UVs coordinates when converting an OBJ to FBX
    [FBXX-1643]
    [FBXX-1639] Security Improvements
    [FBXX-1638] Possible un-processed error condition in the FbxGeometryConverter.

2020.3.1

    [FBXX-1631] Updated libAlembic to version 1.8.2
    [FBXX-1628] Updated libxml2 to version 2.9.12

2020.3.0

    [FBXX-1623] HIK RotationOrder is misinterpreted in DeepConvertScene
    [FBXX-1617] FbxStream is possibly limited to 2GB files

2020.2.3

    [FBXX-1622] Security Improvements

2020.2.2
    * Performance optimizations.
      
      One of the optimizations in this release changed how referenced objects and property pages are stored internally.
      Instead of a linear list, we now use a Red/Black tree. This change means that we cannot guarantee that the 
      retrieval order of these objects will always follow their insertion order (it depends on how they get sorted
      in the tree). Because ot this, we have deprecated the FBxObject::GetReferenceBy(int pIndex) function and added
      FbxObject::GetReferencedBy(FbxArray<FbxObject*>&) which should be used instead to get all the references in 
      one single call.
      
    [FBXX-1618] BeginGetMeshEdgeIndexForPolygon does not fill lookup tables correctly.
    [FBXX-1619] Security Improvements.

2020.2.1
    * Performance optimizations.
    
    * With the introduction of clang 12 compiler required to provide the libraries for the Intel and 
     the M1/Silicon platform, the FBX SDK libraries can only be deployed on 10.15+ systems. Trying 
     to use these libraries on older OSes (10.14 and earlier) would fail with the error:
    
         dyld: lazy symbol binding failed: Symbol not found: ____chkstk_darwin
    
     To resolve this error, the Package of the FBX SDK now includes a separate library directory (legacy_clang)
     that contains a compatible build (x86_64 only) that targets 10.13 as the minimum OS version.

2020.2
    [FBXX-1478] Should have a Up-Axis option when loading a c3d motion file
    [FBXX-1590] Add option flags to disable LayerElement import
    
                WARNING: these flags only work for the FBX file format and should be used in very specific
                         cases where it is known that the skipped data is not needed (or is recomputed by
                         the importing program).
                         
                The following flags have been added to the IOSettings and can be used to disable
                the reading of the corresponding data section in the FBX files (by default they are all
                enabled). They apply to all the objects in the file and can only be changed at runtime.

                    IMP_FBX_NORMAL       skip all LayerElementNormal
                    IMP_FBX_BINORMAL     skip all LayerElementBinormal
                    IMP_FBX_TANGENT      skip all LayerElementTangent
                    IMP_FBX_VERTEXCOLOR  skip all LayerElementVertexColor
                    IMP_FBX_POLYGROUP    skip all LayerElementPolygonGroup
                    IMP_FBX_SMOOTHING    skip all LayerElementSmoothing
                    IMP_FBX_USERDATA     skip all LayerElementUserData
                    IMP_FBX_VISIBILITY   skip all LayerElementVisibility
                    IMP_FBX_EDGECREASE   skip all LayerElementEdgeCrease
                    IMP_FBX_VERTEXCREASE skip all LayerElementVertexCrease
                    IMP_FBX_HOLE         skip all LayerElementHole
                
                    NOTE: turning off IMP_FBX_NORMAL may produce FbxShape geometries that still define empty LayerElementNormal objects
                          (when FbxShape objects are defined with the 'Legacy' mode, normal are not computed at all and both the shapes
                           and the base geometry will not have normals defined).
 
    [FBXX-1592] Update IMP_FBX_ANIMATION to correctly skip all animation data 
                    
                    NOTES: 1. The FbxAnimStack objects are only skipped if both the IMP_FBX_ANIMATION and the IMP_FBX_AUDIO options
                              are turned off (regardless of the presence of audio objects in the file).
                           2. When the IMP_FBX_ANIMATION option is turned off, all the animation data (FbxAnimLayer, FbxAnimCurveNode, 
                              FbxAnimCurve, FbxAnimCurveKey) of every object in the file is skipped. However, because it is mandatory
                              that at least one FbxAnimLayer be defined in an animation stack (when present), the FBX SDK will 
                              automatically add a new empty layer named "Base Layer".
                           3. By skipping the animation objects the FBX SDK cannot guarantee that the scene will evaluate correctly.
                              The values stored in the properties (for example LclTranslation, LclRotation, LclScaling) are overridden by
                              the FbxAnimCurveNode (the "candidate") which, in turn, is overridden by the key values of the animation 
                              curves. Without these objects, whatever value is set in the properties will be used instead.

    * Linux builds are using the GCC 9.3 compiler.
    
    * MacOS builds are using clang 12.0 and produce Universal Binaries containing Intel (x86_64) and M1/Silicon (arm64)
      architectures.
    
    * Visual Studio 2012 and 2013 builds are no longer supported.
      Visual Studio 2015, 2017 and 2019 are supported.
      
    * Python 2.x is no longer shipped as a pre-compiled library in the 'fbxpythonsdk' package.
      You can still compile your own version using the source .sip files available in the 'fbxpythonbindings'
      pacakge.

    [FBXX-1597] string ReplaceAll may return false even though the replacement was executed correctly.
    [FBXX-1596] DeepConvertScene breaks camera LookAt position
    [FBXX-1583] Unable to get UserData.
    [FBXX-1570] Issue with FbxGeometryConverter::MergeMeshes (some data not preserved correctly after merge)
    [FBXX-1569] Performance degradation causing huge memory allocation with some files.

2020.1
    * Added transport of Standard Surface definition.
      (https://github.com/Autodesk/standard-surface)

    * In order to comply to Apple's Notarization requirements that have been enforced starting Feb 3rd 2020,
      the FBX SDK now targets 10.9 as the minimum OS version.

    * Removed the FBX SDK libraries that linked against libstdc++ (these used to be available for the MacOS platform only).

    [FBXX-1561] Fixed a 8 byte memory leak.
    [FBXX-1564] EXP_OBJ_TRIANGULATE flag is ignored during the OBJ write
    [FBXX-1565] FbxGeometryConverter::Triangulate() crashes on some badly formed meshes

2020.0.3
    * VisualStudio 2017 packages now include ARM64 libraries.

      The same rules as the Windows Store ARM 'flavor' libraries shipped with 
      the other VisualStudio compilers also apply to the desktop 'flavor' available
      in this package. This means that, due to sandbox requirements, some specific 
      system calls in the FBX SDK are disabled. And that sample code may not compile
      or work without changes.

    [FBXX-1557] Fixed performance degradation.

2020.0.2
    [FBXX-1545] Fixed regression that caused some 3DS files to be reported as corrupted
    
2020.0.1
    * Officially retired the 32 bits builds of the FBX SDK on MacOS as well as
      the 32 bits flavor of the FBX SDK python bindings for MacOS and Linux.
      
    [FBXX-1542] Correctly handle different rotation orders inside FbxGetContinuousRotation function
    [FBXX-1529] FBX SDK quaternion multiplication may produce NaN values
      
2020.0
    * The FbxShape class has been expanded to handle more channels (BiNormals,
      VertexColor and UV) called "Modern Style". See the class documentation and
      the ShapeAttributes sample code for more information.
      
    * Added new FbxEmbeddedFileCallback class that can be used to modify how
      the FBX SDK get/set embedded data to overwrite the default disk access.
      
    * Officially retired the 32 bits builds except for ARM architecture.

    [FBXX-1537] Security Improvements.
    [FBXX-1525] 3ds import on MacOS fails to parent siblings objects

2019.5.1
    [FBXX-1518] Fix bad rotation and scaling when writing Acclaim files
